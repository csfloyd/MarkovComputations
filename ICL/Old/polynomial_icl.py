...
def generate_icl_gmm_data_with_label_swap(gmm, n_samples, N, exact_copy=True, B=1, K_classes=None):
    ...
    for _ in range(n_samples):
        ...
        # This maps original labels (1 to K) to swapped labels (1 to K)
        ...
        if B == 1:
            ...
            for i in range(N):
                ...
                swapped_label = label_permutation[original_label - 1].item()  # -1 for 0-indexing
...
class MatrixTreeMarkovICL(nn.Module):
    ...
    def __init__(self, n_nodes=10, z_dim=2, K_classes=75, N=4, use_label_mod=False):
        ...
        # Optional: modulate rates by context labels (only if use_label_mod=True)
    ...
    def compute_rate_matrix_K(self, z_batch, labels_batch=None):
        ...
        # Optional: Add label modulation (only if enabled and labels provided)
    ...
    def forward(self, z_seq_batch, labels_seq_batch, method='direct_solve', temperature=1.0):
        ...
        # Labels are 1-indexed, so subtract 1 for 0-indexed one-hot encoding
        labels_one_hot = torch.nn.functional.one_hot(
            labels_seq_batch.long() - 1, 
            num_classes=self.K_classes
        ).float()  # (batch_size, N, K_classes)
        
        # Aggregate attention weights by label class
        # For each class k, sum attention weights where label = k
        # Using einsum: (batch, N) × (batch, N, K) → (batch, K)
        logits = torch.einsum('bn,bnk->bk', attention, labels_one_hot)  # (batch_size, K_classes)
        
        # Convert to log-probabilities for NLLLoss
        # Clamp to prevent catastrophic loss spikes when model assigns very low probability to correct class
        # Using 1e-6 instead of 1e-10 prevents log(~0) ≈ -23 which causes massive loss spikes
...
def train_model(model, train_loader, val_loader, device, n_epochs=200, lr=0.001, method='direct_solve', temperature=1.0,
                gmm=None, N=None, B=1, K_classes=None, exact_copy=True, eval_frequency=10, n_eval_samples=500):
    """
    ...
    # Use NLLLoss since model outputs log-probabilities
    ...
    history = {
        ...
        'iwl_acc': [],           # In-Weight Learning (no target class in context)
        'icl_novel_acc': [],     # ICL Primary: Novel classes with B copies
    ...
    for epoch in range(n_epochs):
        ...
        if gmm is not None and N is not None and (epoch + 1) % eval_frequency == 0:
            ...
            # 1. IWL Accuracy: Target class unlikely to appear in context (B=1, independent sampling)
            # This tests if model learned the class-label mapping in weights
            iwl_data = generate_icl_gmm_data(gmm, n_eval_samples, N, novel_classes=False, 
                                            exact_copy=False, B=1, K_classes=K_classes)
            iwl_correct = 0
            with torch.no_grad():
                for z_seq, labels, target in iwl_data:
                    logits = model(z_seq.unsqueeze(0).to(device), labels.unsqueeze(0).to(device), 
                                 method=method, temperature=temperature)
                    pred_class = logits.argmax(dim=1).item() + 1
                    if pred_class == int(target.item()):
                        iwl_correct += 1
            ...
            # This tests pure in-context learning on unseen classes
            icl_novel_data = generate_icl_gmm_data(gmm, n_eval_samples, N, novel_classes=True, 
            ...
            # 3. ICL Secondary: Label swapping (trained classes but swapped labels)
            # This tests if model can learn new label mappings in-context
            icl_swap_data = generate_icl_gmm_data_with_label_swap(gmm, n_eval_samples, N, 
                                                                  exact_copy=exact_copy, B=B, K_classes=K_classes)
            icl_swap_correct = 0
            with torch.no_grad():
...
#                          gmm=gmm, N=args.N, B=args.B, K_classes=args.K_classes, 
#                          exact_copy=args.exact_copy, eval_frequency=10, n_eval_samples=500)
